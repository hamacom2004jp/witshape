...
class PgvectorInstall(feature.Feature):
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float, pf:List[Dict[str, float]]=[]) -> Tuple[int, Dict[str, Any], Any]:
        ...
        try:
            ...
            returncode, output, cmd = common.cmd(f"docker-compose -f {dist_path}/docker-compose.yml build", slise=-1)
            if returncode != 0:
                ret = dict(error=f"install error: {output}")
                logger.error(f"install error: {output}")
            ret = dict(success=f"install success: {output}")
            logger.info(f"install success: {output}")
